...
function wordsList(str, subStr) {
    let myLongStr = str;
      for(i = 0; i < myLongStr.length; ++i) {
        if(myLongStr[i] === ',' || myLongStr[i] === '.' || myLongStr[i] === '!'
            || myLongStr[i] === '?' || myLongStr[i] === ';' || myLongStr[i] === '"' 
            || myLongStr[i] === ':') {
                myLongStr = myLongStr.replace(myLongStr[i], '');
        }
        
      };
            
     myLongStr = myLongStr.toLowerCase();
      let arr = myLongStr.split(' '); 
      arr = arr.filter((el) => el.indexOf(subStr) > -1);
      let set = new Set();
...
function getLocalDate(data, isSeconds = false, isISO = false) {
    let myDate = data;
    ...
    let M = addLeadingZero(myDate.getMonth() + 1);
    let D = addLeadingZero(myDate.getDate());
    let h = addLeadingZero(myDate.getHours());
    let m = addLeadingZero(myDate.getMinutes());
    let s = addLeadingZero(myDate.getSeconds());
    let res;
if (isSeconds === true && isISO === false ) { 
    res = D + '.' + M + '.' + Y  + ', ' + h + ':' + m + ':' + s;
} else if (isSeconds === false && isISO === true ) { 
    res = Y + '-' + M + '-' + D  + ', ' + h + ':' + m;
} else if (isSeconds === true && isISO === true ) { 
    res = Y + '-' + M + '-' + D  + ', ' + h + ':' + m + ':' + s;
} else { 
    res = D + '.' + M + '.' + Y  + ', ' + h + ':' + m;
} 
...
let data = new Date(date);
let current = data.getTime();
let amount = days * 24 * 60 * 60 * 1000;
let newDate = current - amount;
let res = new Date(newDate);
...
let M = addLeadingZero(res.getMonth() + 1);
...
let Car = function (engine, model, name, year, used) {
        this.name = name;
        this.model = model;
        this.engine = engine;
        this.year = year;
...
Object.defineProperties(Car.prototype,{
    used: {
        ...
        set(value) {
            if (this.year < yearNow && value === 'new') {
            this.year = yearNow;
          }
        }
     }
